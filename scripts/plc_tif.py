...
def readrpm(address):
  stream = os.popen('ssh root@192.168.0.171 "clia sensordata '+address+' 0:1|grep Processed|cut -b 21-24"')
  temp_cc=""
  for i in stream.read():
    temp_cc+=i
  return float(temp_cc)
...
def readexttemp():
  stream = os.popen('ssh root@192.168.0.171 "clia sensordata 0x14 1:11|grep Processed|cut -b 21-25"')
  temp_cc=""
  for i in stream.read():
    temp_cc+=i
  return float(temp_cc)
...
while raw_in.strip() != 'e':
	...
	for k in range(int(trange_opt[0]),int(trange_opt[1])):
	 ...
	 img4.FillRectangle("#"+"%0.2x"%(int(rgb[0]*255))+"%0.2x"%(int(rgb[1]*255))+"%0.2x"%(int(rgb[2]*255)), 20+(100-rangeconv(trange_opt,k))*6,100, int(trange_opt[1]-trange_opt[0]/100*6), 50)
	textc1_opt=TText(.01,.95,str(trange_opt[0])+" \xB0"+"C")
	textc2_opt=TText(.01,.95,str(trange_opt[1])+" \xB0"+"C")
	textopt=TText(.01,.95,"Optics")
	img4.DrawText(textc1_opt,20,200)
	img4.DrawText(textc2_opt,20+100*6,200)
	img4.DrawText(textopt,10+50*6,140)
	img4.Draw()
	...
	textexa1=TText(.01,.95, "Exhaust temp 0xcc="+str(readtemp("0xcc"))+" \xB0"+"C")
	textexa2=TText(.01,.95, "Exhaust temp 0xc8="+str(readtemp("0xc8"))+" \xB0"+"C")
	textexa3=TText(.01,.95, "Exhaust temp 0xca="+str(readtemp("0xca"))+" \xB0"+"C")
	textexa4=TText(.01,.95, "Exhaust temp 0xce="+str(readtemp("0xce"))+" \xB0"+"C")
	textrpm1=TText(.01,.95, "RPM 0xcc="+str(readrpm("0xcc")))
	textrpm2=TText(.01,.95, "RPM 0xc8="+str(readrpm("0xc8")))
	textrpm3=TText(.01,.95, "RPM 0xca="+str(readrpm("0xca")))
	textrpm4=TText(.01,.95, "RPM 0xce="+str(readrpm("0xce")))
	...
	img4.DrawText(textrpm1,750,320)
	img4.DrawText(textrpm2,750,360)
